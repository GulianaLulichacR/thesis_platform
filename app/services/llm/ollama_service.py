"""
OllamaService — updated to use new config fields.

Changes vs. original:
  - Reads OLLAMA_MODEL from settings (was hardcoded as 'llama3').
  - Reads OLLAMA_ENABLED flag — construction raises if disabled.
  - provider_priority = 2 (second in free chain after Gemini).
  - All error handling / URL sanitisation kept intact.
"""
...
class OllamaService(BaseLLMService):
    ...
    Best free Ollama models by use-case:
      - llama3 / llama3:8b       — best quality, needs ~6 GB RAM
      - llama3:latest            — same, pulled automatically
      - mistral:7b               — fast, great for instruction following
      - phi3:mini                — 3.8B, very fast, good for low-RAM (~2.5 GB)
      - qwen2:1.5b               — 1.5B, CPU-only viable, ~1 GB RAM
      - nomic-embed-text         — embeddings only (for RAG)

    CPU-only recommendations:
      - phi3:mini  (recommended, best quality/RAM trade-off)
      - qwen2:1.5b (if RAM is very constrained)
    """
