...
class LocalStorageService:
    ...
    def get_metadata_path(self, file_id: str) -> Path:
        return self._base_dir / f"{file_id}_meta.json"

    async def save_metadata(self, file_id: str, metadata: dict) -> None:
        path = self.get_metadata_path(file_id)
        try:
            async with aiofiles.open(path, "w", encoding="utf-8") as f:
                await f.write(json.dumps(metadata, indent=2))
        except OSError as exc:
            raise StorageError(f"Could not save metadata: {exc}") from exc

    async def get_metadata(self, file_id: str) -> dict:
        path = self.get_metadata_path(file_id)
        if not path.exists():
            raise StorageError(f"Metadata for '{file_id}' not found.")
        try:
    ...
    async def update_metadata(self, file_id: str, updates: dict) -> dict:
